...
| File | Role |
|------|------|
| `terminal.py` | CLI: voice/text → brain → TTS, wake word loop, proactive engine. Textual "control console" TUI by default on real terminals (ctrl+1..7 switch panes — digit keys never collide with TextArea editing, unlike ctrl+w/v which are word-delete/paste there; ctrl+r record, ctrl+c copy/cancel in composer, ctrl+q quit; composer bridges the real macOS clipboard — cmd+v/super+v and ctrl+v paste it (Textual's internal clipboard is bypassed), cmd+c/ctrl+c push selections out to it; ctrl+shift+c / ctrl+shift+t / ctrl+shift+a copy the last JARVIS reply / the whole transcript / the last ~400 thinking lines straight to the OS clipboard (priority bindings, work even while the composer is focused; hint lives in the status strip); Catppuccin Mocha theme); `JARVIS_TUI=0` forces the classic REPL, `JARVIS_TUI_VISION=0` skips camera autostart, `JARVIS_TUI_ANNOUNCE=1` speaks flavor lines, `JARVIS_CAMERA_INDEX=<0-4>` forces the vision camera (auto-pick prefers whatever camera actually delivers frames — virtual/Continuity cameras that only produce black frames are skipped; camera streams only while the VISION tab is open). **Classic-REPL input runs in non-canonical tty mode** (`_RawTTYLineReader`): canonical mode caps a line at 1024 bytes and silently drops the excess, so long single-line pastes (no `\n`) were truncated at 1024 chars before Jarvis read them (first paste with paragraph breaks survived; the one-line variant lost its tail). Raw mode lifts that — paste length is bounded only by `JARVIS_PASTE_MAX_CHARS` (100k) — while hand-rolling echo/backspace/UTF-8, ^C → KeyboardInterrupt, ^D → flush-or-EOF, and preserving burst-batching. `JARVIS_TTY_RAW=0` restores the canonical `input()` path; piped stdin is untouched (still one line). PTY regression coverage in `tests/test_tty_paste.py`. The SYSTEM tab is a mini Activity Monitor: live per-process table (2s refresh only while the tab is open), keys c/m/d/n/e switch the metric like Activity Monitor's tabs — CPU / MEM / DISK (system-wide I/O rates) / NET (system-wide rates; macOS psutil has no per-process net/disk counters, so those two show the busiest-by-CPU processes underneath) / ENERGY (CPU-weighted EMA). Shell aliases: `js1` = classic REPL (`JARVIS_TUI=0`), `je1` = control console (`JARVIS_TUI=1`) — defined in `~/bin/js1` and `~/bin/je1` (on PATH via `~/.zshrc`; run `rehash` or open a new tab after editing them) |
...
| `brain.py` | Provider chain, tool calling, safety gating, tool definitions, intent router, circuit breaker, `_tool_call_names` tracking, `get_last_tool_calls()`. **CAD intent is keyword-gated on unambiguous signals only** (`_cad_triggered`): Onshape URLs, strong tokens (`onshape`, `cad`, `part studio`) alone, weaker CAD verbs (`extrude`/`fillet`/`chamfer`) only with a second CAD keyword. Generic engineering words (`feature`, `depth`, `part`, ...) never route to `cad` — they misrouted chat/coding (e.g. "visual feature extraction", "arbitrary depth"). `_handle_cad_request` returns `None` (falls through to the provider chain) when the message has no document URL/IDs instead of dead-ending in canned Onshape help |
| `agent.py` | Agent loop (Think→Act→Evaluate + planner) with **loop discipline budgets** (Phase 2): hard per-request limits — `JARVIS_AGENT_MAX_TOOL_CALLS` (10 total tool executions), `JARVIS_AGENT_MAX_EXPLORE` (8 read-only calls from `EXPLORE_TOOLS`; excess blocked with a forced phase advance), `JARVIS_AGENT_WALL_CLOCK_S` (90s ceiling). Forced monotonic phase contract EXPLORE→PLAN→IMPLEMENT (model may set `"phase"` in its JSON decision; regressions are clamped; plan auto-advances after 2 steps); `max_iterations` clamped to tool budget + 4. Duplicate tool+args calls, per-tool fail counts (≥2), and high-frequency tools are skipped. Empty/`null` model replies are retried once, then the loop **aborts gracefully with a partial-progress summary** (`_synthesize_from_steps`) instead of "No response from model." — `_parse_decision` guards non-dict JSON (`json.loads("null")` crash fixed). **Planner JSON robustness (Phase 4)**: `_create_plan` parses strict JSON, fenced (` ```json `) blocks, and prose-wrapped arrays via `_extract_json_array`/`_extract_json_object` (string-aware bracket scanning — brackets inside string values don't truncate); malformed step entries are dropped (`_step_from_dict` schema hardening: unknown keys stripped, non-dict args coerced) instead of crashing the loop; a plain-completion (prose) planner reply degrades to the canned "couldn't create a plan" message with no tool executed and no `subagent_completed` event. Regression tests in `tests/test_agent_loop.py` + `tests/test_planner_json.py` |
...
| 3 | **Groq groq/compound-mini** | Fallback — text-only by default: `JARVIS_GROQ_TOOLS=1` re-enables tool calling (compound-mini 400s "tool calling is not supported" otherwise) |
...
### Phase 3: permanent retirement, request budget, Groq gate

- **Permanent retirement** (`brain.py`): 401/402/403 and payment-wall errors ("payment required", "insufficient credits", "no credits", "billing", "accounts have insufficient balance") classify `permanent` — the provider is retired in-memory + persisted under the `__retired__` meta key in the health file, so every failover request stops re-hitting dead payment walls (observed: 11× 402 in one session). Retirement is skipped under mock mode (`JARVIS_MOCK_PROVIDERS=1` — test 402s must not retire real providers); a successful reply un-retires the provider. Exposed via `get_provider_health()` (`retired`/`retired_reason`).
- **Per-request attempt budget**: `JARVIS_PROVIDER_REQUEST_BUDGET` (default 6) caps total provider attempts per request (read at import; `threading.local()` counter) — gates the latency chain, nemotron primary, policy primary, plugin loop, and fallback loop.
- **Groq gate**: `JARVIS_GROQ_TOOLS` (default `0`) — Groq's compound-mini 400s with tools; the OpenAI-compatible path forces `enable_tools=False` for Groq unless allowed. OpenRouter keeps tools.
...
| Runaway `caffeinate` processes | Every JARVIS process start (terminal/app/server) spawned one `caffeinate -i` and never stopped it (≤1013 leaked → hits the per-UID process cap) | Fixed Phase 3.5: idempotent `start_caffeinate()` + `atexit` cleanup + skip under `JARVIS_EVAL_MODE=1`; `pkill -f caffeinate` clears old orphans |
